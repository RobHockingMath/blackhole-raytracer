...
//initialize world
	...
	#pragma omp parallel num_threads(numThreads)
    {
		...
		char* photon_map_2 = "Object_3_Geola.txt"; // temporary fix - photon map 2 is all black.
...
//render world and write result to file
	...
	string filename_base = "./Oppenheimer_final2/";
	string time_base = "./Oppenheimer_final2/time_";
	
	string dimness_base = "./Oppenheimer_final2/dimness_";
	
	
	string filename_middle = "hogelOut";
	string filename_extension = ".bmp";
	string filename_extension_txt = ".txt";
	string underscore = "_";
...
try { 
	...
	//#pragma omp parallel num_threads(numThreads)
    {
		...
		//#pragma omp critical
			...
			cout<<"water texture 2 done"<<endl;
	...
	//&&	!(2250<=i && i<=32315) && !(2500<=i && i<=2573) && !(2750<=i && i<=2839) && !(3000<=i && i<=3100) && !(3250<=i && i<=3369) && !(3500<=i && i<=3646) && !(3750<=i && i<=3899)){
		//if((i==2639)){
			...
			P0->upper_bound=front+LL*tan(fabs(rot_angle));
			P0->upper_bound_variable=2;
			P0->upper_bound_coefficient=tan(fabs(rot_angle));
			...
			//P1->upper_bound=3.0;
			//P1->upper_bound=front+LL*tan(fabs(rot_angle));
			//P1->upper_bound_variable=2;
			//P1->upper_bound_coefficient=tan(fabs(rot_angle));
			...
			InfinitePlane* P3 = new InfinitePlane(2,0,255,255);
			P3->coordinate = 0;
			P3->texture_ID=0;
			...
			P5->upper_bound=front+LL*tan(fabs(rot_angle));
			P5->upper_bound_variable=2;
			P5->upper_bound_coefficient=tan(fabs(rot_angle));
			//P5->upper_bound=3;
			//P5->upper_bound=2;
			...
			P6->upper_bound=front+LL*tan(fabs(rot_angle));
			P6->upper_bound_variable=2;
			P6->upper_bound_coefficient=tan(fabs(rot_angle));
			//P6->upper_bound=3;
			//P6->upper_bound=2;
			...
			//double theta_chimera = pi/6+i*(pi-2*pi/6)/N;
			double theta_chimera = i*(pi)/(N-1);
			//double theta_chimera = pi/2;
			...
			cout<<"EE2 = "<<EE2[0]<<" "<<EE2[1]<<" "<<EE2[2]<<endl;
			
			//p[0]=2+r_geola*sin(theta_chimera);
			////p[1]=r_geola*cos(theta_chimera);
			//p[2]=-3;
			...
			//double P_obs_loc[4]={30+i*10.0/N,r_geola,0,1};
			...
			Pinhole_Camera my_Camera(p,l,u,F_chimera);
			...
			// does doesn't work.
			double black_hole_mass=0.075*alpha;
			//black_hole_mass *=0;
			//double black_hole_pos[3]={2.0-6*black_hole_mass,0,-3};
			//double black_hole_pos[3]={1,0,-3};
			//double black_hole_pos[3]={0.0,0,-2};
			double black_hole_pos[3]={0.0,0,0};
			//double black_hole_pos[3]={2.0,0,-3};
			//black_hole_mass = 1e-6;
			//black_hole_mass = 0;
			int dimension = 4;
			...
			//double R00=6.0*pow(alpha,1.0/3);
			//double R00=6.0*alpha;
			double R00=6.0*pow(alpha,1.0/3);
			cout<<"making spacetime"<<endl;
			Oppenheimer_Snyder_Collapse* Universe = new Oppenheimer_Snyder_Collapse(black_hole_pos,error_tol,black_hole_mass,R00);
			cout<<"finished making spacetime"<<endl;
			
			earth.space_time = Universe;
			...
			//earth.render(&my_Camera,filename_i,w,h);

			/**N=100;

			for(int ii=0;ii<N;ii++){
				string time_str = time_base+to_string(ii)+filename_extension_txt;
				char *time_i = new char[time_str.length()+1];
				strcpy(time_i, time_str.c_str());
				cout<<time_i<<endl;
				double P_obs_loc[4]={30+ii*10.0/N,r_geola,0,1};
				earth.spherically_symmetric_fartime_table(P_obs_loc,time_i);

			}**/
			cout<<"about to render"<<endl;
			string time_str = time_base+to_string(i)+filename_extension_txt;
			string dimness_str = dimness_base+to_string(i)+filename_extension;
			char *time_i = new char[time_str.length()+1];
			strcpy(time_i, time_str.c_str());
			char *dimness_i = new char[dimness_str.length()+1];
			strcpy(dimness_i, dimness_str.c_str());
			//double P_obs_loc[4]={30.0+i*10.0/N,r_geola,0,1};
			
			double time_interval = 0*10*(95.0/100)*(8.5/10);
			time_interval = 2*3.7;
			//double time_interval = 10*(95.0/100)*alpha;
			//double time2 = 36+1-time_interval/2+i*time_interval/(N-1);
			//double time2 = 36+1-time_interval/2+i*time_interval/(N-1);
			double time2 = 36+2.0-time_interval/2+(i-200)*time_interval/(N-401);
			
			if(time2<36.0+2.0-time_interval/2){
				time2 = 36.0+2.0-time_interval/2;
			}
			if(time2>36.0+2.0+time_interval/2){
				time2 = 36.0+2.0+time_interval/2;
			}
			
			double distance2 = sqrt((p[0]-black_hole_pos[0])*(p[0]-black_hole_pos[0])+(p[1]-black_hole_pos[1])*(p[1]-black_hole_pos[1])+(p[2]-black_hole_pos[2])*(p[2]-black_hole_pos[2]));
			
			//double P_obs_loc[4]={31.0+i*10.0/N,r_geola,0,1};
			//double P_obs_loc[4]={time2,r_geola,0,1};
			double time_delta = fabs(distance2-r_geola);
			double P_obs_loc[4]={time2+time_delta,distance2,0,1};
			//earth.spherically_symmetric_dimmness_image(&my_Camera,dimness_i,time_i,w,h,P_obs_loc,V_obs_loc);
			//earth.render_spherically_symmetric_spacetime(&my_Camera,black_hole_pos,filename_i,time_i,w,h,P_obs_loc,V_obs_loc);
			earth.save_sample_lightrays(black_hole_pos,P_obs_loc,black_hole_mass,i);
