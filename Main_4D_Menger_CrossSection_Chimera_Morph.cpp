...
//define a bunch of lights
	...
	const int numThreads = 16;  // Explicitly set the number of threads
	...
	#pragma omp parallel num_threads(numThreads)
    {
		...
		char* gold_spiral_texture="Orange_little.txt";
		#pragma omp critical
			{
...
try { 
	...
	//&&	!(2250<=i && i<=32315) && !(2500<=i && i<=2573) && !(2750<=i && i<=2839) && !(3000<=i && i<=3100) && !(3250<=i && i<=3369) && !(3500<=i && i<=3646) && !(3750<=i && i<=3899)){
		//if((i==2639)){
			...
			P1->reflectivity=0.5;
			//Parallelogram *P1=new Parallelogram(o,x1,x2,187,185,39);
			//P1->transparency=0.5;
			...
			earth.env_map_ID=0; //the texture ID of the background image - explained later
