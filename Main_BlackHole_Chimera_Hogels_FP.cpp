...
//define a bunch of lights
	...
	const int numThreads = 2;  // Explicitly set the number of threads
...
//render world and write result to file
	...
	string filename_base = "./Chimera_300x400_direct_Hogel_FP/";
...
try { 
	...
	for(int b=938;b<1000;b++){ 
	//for(int i=126;i<168;i++){ 
		
		string angle = to_string(b);
		if(b<10){
			angle = "00000"+angle;
		}
		else if(b<100){
		...
		//#pragma omp parallel for private(p,l,u,w,h) num_threads(numThreads) schedule(dynamic)
		for(int a=600;a<=600;a++){
		string slash = "/";
		string folder = to_string(a)+slash;
	...
	//&&	!(2250<=i && i<=32315) && !(2500<=i && i<=2573) && !(2750<=i && i<=2839) && !(3000<=i && i<=3100) && !(3250<=i && i<=3369) && !(3500<=i && i<=3646) && !(3750<=i && i<=3899)){
		//if((i==2639)){
			...
			string filename_str = filename_base+to_string(b)+filename_extension;
			...
			double theta_chimera = a*pi/1200;
			...
			double theta_SLM_h = 120*pi/180;
			double theta_SLM_v = 90*pi/180;
			...
			earth.render_full_parallax_hogel(&my_Camera, filename_i, a, b, N_a, N_b, N_I, N_J, black_hole_pos, Ly, Lz, r_geola, theta_SLM_h,theta_SLM_v);
