...
double Psi0=0.00045;
//Psi0 = 0.5*Psi0;
double r0=5; //led to horizon at u = 4.0025
//r0=2.5;
//double r0=50;
double delta=0.3; //for r0=5
//double delta=3;
//int n=800;
int n=1600;
double R_max=200; // 4000 resulted in a crash
//double u_max=25;
double u_max=35;
//double u_max=250;
double u_min=0;
...
double r0p=7;
double Psi0p=Psi0/60; // went with r0=5
//double Psi0p=Psi0/100; // was 4

double error_tol=1e-6;

Gravitational_Collapse* Universe = new Gravitational_Collapse(Psi0,r0,delta,u_min,u_max,n,R_max,error_tol);
//Universe->Solve_Einstein_Equations();
//Universe->write_stats_to_disk();
string universe_filename = "BlackHole_EH_47_delta_0p5_3200.raw";
Universe->readSpacetimeFromDisk(universe_filename);
Universe->m_bh=Universe->get_hawking_mass(Universe->U[Universe->column_count-2],0.5*(Universe->v_min+Universe->v_max));
cout<<endl<<" We are done building our spacetime.";

if(Universe->contains_blackhole){
	cout<<"  Please note that this spacetime contains a black hole, with horizon at u="<<Universe->u_horizon<<"."<<endl<<endl;
	cout<<"  The black hole has mass "<<Universe->m_bh<<endl;
}
else{
	cout<<" There is no black hole in this spacetime."<<endl<<endl;
}
...
double u0=0.5;
//double v0=7;
double v0=u0+2*r_desired;
double tau_max = 10.0;
//vector<double> u_path,v_path,r_path,tau_path,m_path;
//Universe->constant_r_trajectory(u0,v0,tau_max,u_path,v_path,r_path,tau_path,m_path);

string observer_filename = "Infall_Observer_smoothed.txt";
Universe->load_path(observer_filename); //[tau, u_smooth, v_smooth, u_dot_smooth,v_dot_smooth];
int N_data = Universe->reference_data_length;

//double tau_max = Universe->reference_data[reference_data_length-1]

// Open a file for writing
//std::ofstream outFile("test_trajectory.txt");
//if (!outFile) {
...
//for (int i = 0; i < tau_path.size(); i++) {
//	outFile << tau_path[i] << "\t" << u_path[i] << "\t" << v_path[i] << "\t" << r_path[i] << "\t" << m_path[i] << "\n";
	//cout << tau_path[i] << " " << u_path[i] << " " << v_path[i] << endl;
//}

// Close the file
//outFile.close();
//std::cout << "Data written to test_trajectory.txt" << std::endl;
...
//render world and write result to file
	...
	string filename_base = "./Gravitational_Collapse/";
	string time_base = "./Gravitational_Collapse/time_";
...
try { 
	...
	//for(int i=126;i<168;i++){ 
		...
		double uu, vv, uup,vvp;
		
		
		double i_data = i*N_data/double(N);
		int i_data_m = max(int(floor(i_data)),0);
		int i_data_p = min(i_data_m+1,N-1);
		double ss = i_data-floor(i_data);
		uu = Universe->reference_data[i_data_m][1]*(1-ss)+Universe->reference_data[i_data_p][1]*ss;
		vv = Universe->reference_data[i_data_m][2]*(1-ss)+Universe->reference_data[i_data_p][2]*ss;
		uup = Universe->reference_data[i_data_m][3]*(1-ss)+Universe->reference_data[i_data_p][3]*ss;
		vvp = Universe->reference_data[i_data_m][4]*(1-ss)+Universe->reference_data[i_data_p][4]*ss;
	...
	//&&	!(2250<=i && i<=32315) && !(2500<=i && i<=2573) && !(2750<=i && i<=2839) && !(3000<=i && i<=3100) && !(3250<=i && i<=3369) && !(3500<=i && i<=3646) && !(3750<=i && i<=3899)){
		//if((i==2639)){
			...
			double P_obs_loc[3]={uu,vv,0};
			double V_obs_loc[3]={uup,vvp,0};
			...
			//earth.spherically_symmetric_fartime_table(P_loc,time_i);
			cout<<"about to render"<<endl;
			earth.render_spherically_symmetric_spacetime(&my_Camera,filename_i,time_i,w,h,P_obs_loc,V_obs_loc);
