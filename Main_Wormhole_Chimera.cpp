...
//render world and write result to file
	...
	//string filename_base = "./Chimera_300x400_new4/";
	string filename_base = "./Wormhole/";
	string filename_middle = "hogelOut";
	string filename_extension = "_SoftShadedCube.bmp";
	string underscore = "_";
...
try { 
	...
	//&&	!(2250<=i && i<=32315) && !(2500<=i && i<=2573) && !(2750<=i && i<=2839) && !(3000<=i && i<=3100) && !(3250<=i && i<=3369) && !(3500<=i && i<=3646) && !(3750<=i && i<=3899)){
		//if((i==2639)){
			...
			//array<double, 3> bh_pos1 = {0.0, 1.5, -3.0};
			//array<double, 3> bh_pos2 = {0.0, -1.5, -3.0};
			array<double, 3> bh_pos1 = {x00, y00, -3.0};
			array<double, 3> bh_pos2 = {x00, -y00, -3.0};
			vector<array<double, 3>> bh_positions = {bh_pos1, bh_pos2};
			vector<double> masses = {0.6, 0.6};
			vector<int> stitching = {1, 0};
			
			Multi_black_hole* BlackHoles = new Multi_black_hole(bh_positions,error_tol,masses,stitching);
			//Multi_worm_hole* WormHoles = new Multi_worm_hole(bh_positions,error_tol,masses,stitching);
			//earth.space_time=WormHoles;
			earth.space_time=BlackHoles;
			//earth.space_time=FlatSpace;
			//earth.space_time=BlackHole;
			/**double C = offset4D;
			int type = 1;
